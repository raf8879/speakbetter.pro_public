  ...
  export default function PracticeWordsPage() {
    ...
    /**
     * Храним идентификатор интервала в ref, чтобы позже его очистить.
     */
    type PollTimer = ReturnType<typeof setInterval>;
    const pollRef = useRef<PollTimer | null>(null);
  
    /**
     * Функция, которая грузит список «плохих слов» с бэкенда
     */
    const fetchMisWords = () => {
      api
    ...
    useEffect(() => {
      // 1) Пока загружается AuthContext, не делаем запрос
      if (isLoading) return;
  
      // 2) Если пользователь не авторизован — редирект
      if (!user) {
        router.push("/login");
        return;
      }
  
      // 3) Иначе первый раз грузим слова
      fetchMisWords();
